...
parser.add_argument('--number-of-cpus',
                    required=True,
                    type=int)
parser.add_argument('--number-of-gpus',
                    required=True,
                    type=int)
...
training_dataset, number_of_training_steps = generate_dataset(subset_associations=subset_associations['train'],
                                                              input_size=INPUT_SIZE,
...
validation_dataset, number_of_validation_steps = generate_dataset(subset_associations=subset_associations['train'],
                                                                  input_size=INPUT_SIZE,
                                                                  number_of_epochs=NUMBER_OF_EPOCHS,
                                                                  batch_size=1,
                                                                  number_of_cpus=NUMBER_OF_CPUS,
...
for epoch in range(FIRST_EPOCH, NUMBER_OF_EPOCHS):
    ...
    for current_step_index in range(number_of_training_steps):
        for k in range(NUMBER_OF_GPUS):
            with tf.device('/gpu:{}'.format(k)):
                images_batch, annotations_batch = session.run(next_training_batch)

                # Perform training.
                _, current = session.run([opt, loss],
                                         feed_dict={input_tensor: images_batch, output_tensor: annotations_batch})
    ...
    if epoch % VALIDATE_EVERY == 0:
        ...
        for validation_step in tqdm(range(number_of_validation_steps)):
            for k in range(NUMBER_OF_GPUS):
                with tf.device('/gpu:{}'.format(k)):
                    images_batch, annotations_batch = session.run(next_validation_batch)
