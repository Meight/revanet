...
parser.add_argument('--model-name',
                    type=str,
                    default="BiSeNet",
                    choices=['BiSeNet', 'DeepLabv3_plus', 'GCN'],
...
for epoch in range(FIRST_EPOCH, NUMBER_OF_EPOCHS):
    ...
    for current_step_index in range(number_of_steps):
        ...
        for current_batch_index in range(BATCH_SIZE):
            current_index = (
                current_step_index * BATCH_SIZE +
                current_batch_index) % number_of_used_training_samples
            ...
            output_image_path = random.choice(
                subset_associations['train'][input_image_path])
            ...
            with tf.device('/cpu:0'):
                if not IS_MULTI_LABEL_CLASSIFICATION:
                    input_image, output_image = augment_data(
                        input_image, output_image, input_size=INPUT_SIZE)
                else:
                    # Output tensor is already resized at this point, only resize input image.
                    input_image, _ = utils.resize_to_size(
                        INPUT_SIZE, desired_size=INPUT_SIZE)
        ...
        _, current = session.run([opt, loss],
                                 feed_dict={
                                     input_tensor: input_image_batch,
                                     output_tensor: output_image_batch
                                 })
    ...
    if epoch % VALIDATE_EVERY == 0:
        ...
        for image_index in validation_indices:
            ...
            output_image_path = subset_associations['validation'][
                input_image_path][0]

            if not IS_MULTI_LABEL_CLASSIFICATION:
                ground_truth = utils.load_image(output_image_path)

                input_image, ground_truth = utils.resize_to_size(
                    input_image, ground_truth, desired_size=INPUT_SIZE)
                ground_truth = segmentation.one_hot_to_image(
                    segmentation.image_to_one_hot(ground_truth, class_colors))
                ...
                output_image = session.run(
                    predictions_tensor, feed_dict={input_tensor: input_image})
                ...
                if VALIDATION_THRESHOLD != 0:
                    output_image = segmentation.apply_threshold_to_prediction(
                        output_image, threshold=VALIDATION_THRESHOLD)
                else:
                    output_image = segmentation.one_hot_to_image(output_image)
